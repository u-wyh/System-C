...
void log_line(LogLevel lvl,const string& msg){
    ...
    cout<<"["<<now_time()<<"]"<<"["<<lvl_str<<"]"<<msg<<endl;
}
...
public:
    ...
    void start(){
        ...
        if(state!=State::INIT){
            log_line(LogLevel::ERROR,"Service cannot start,current state: "+state_to_string(state));
            return ;
        }
        state=State::RUNNING;
        ...
        mainThread=thread([this](){
            ...
            while(true){
                ...
                pool.submit([counter](){
                    log_line(LogLevel::DEBUG,"Executing Task "+to_string(counter));
...
public:
    ...
    void close_client(int fd){
        ...
        close(fd);
        buffers.erase(fd);
        log_line(LogLevel::INFO,"[-] client close fd="+to_string(fd));
    }
    ...
    void process_buffer(int fd){
        ...
        while((pos=buffer.find('\n'))!=string::npos){
            ...
            pool.submit([fd,msg](){
                ...
                log_line(LogLevel::DEBUG,"[Worker] thread="+ss.str()+" start proccesing fd = "+to_string(fd)+" msg="+msg);
            });
            string echo=msg+"\n";
