...
public:
    ...
    void log_service(const string &msg){
    ...
    void worker(int id){
        ...
        while(running){
            try{
                if(rand()%5==0){
                    throw runtime_error("random worker error");
                }
                cout<<string("[Worker "+to_string(id)+"] log"+to_string(++cnt[id]))<<endl;
            }
            catch(const exception &e){
                log_service("[Worker "+to_string(id)+"] exception: "+string(e.what()));
            }
    ...
    void tcp_service(){
        ...
        while(running){
            ...
            try{
                ...
                log_service("[TCP error] "+string(e.what()));
            }
            if(client_fd!=-1){
                ::close(client_fd);
            }
    ...
    void record(){
        ...
        while(running){
            // log_service("[Logger] heartbeat\n");
            cout<<"[Logger] heartbeat\n";
            this_thread::sleep_for(chrono::seconds(1));
        }
    }
    ...
    void start(){
        ...
        running=true;
        try{
            workers.emplace_back(&Service::record,this);
            workers.emplace_back(&Service::tcp_service,this);
            cnt.resize(workCount+1);
            for(int i=1;i<=workCount;i++){
                cnt[i]=0;
                workers.emplace_back(&Service::worker,this,i);
            }
        }
        catch(...){
            running=false;
        ...
        log_service("[Service] started with "+to_string(workCount)+" workers");
    }
    ...
    void run(){
        ...
        while(running){
            if(seconds%10==0){
                log_service("[Service] run "+to_string(seconds)+" s");
            }
            this_thread::sleep_for(chrono::seconds(1));
            seconds++;
        }
