...
void client_worker(const string &ip,int port,int id,int message_count){
...
int main(){
    ...
    for(int i=1;i<=threads;i++){
        workers.emplace_back(client_worker,"127.0.0.1",8080,i,message_count);
    }
    ...
    thread monitor([&](){
        ...
        while(g_running){
            ...
            cout<<"[Client Monitor]: send="<<cur_send<<" received="<<cur_recv<<" TPS="<<(cur_send-last_send)<<endl;
            last_send=cur_send;
        }
    });
