...
public:
    ...
    void log(LogLevel lvl, const string &msg) {
        if(lvl<level) return;
        string lvl_str;
        switch(lvl){ case LogLevel::DEBUG: lvl_str="DEBUG"; break;
...
private:
    void process() {
        ...
        while(running || !buffer1.empty() || !buffer2.empty()) {
            ...
            cv.wait_for(lock, chrono::milliseconds(flush), [this](){ return !buffer1.empty() || !buffer2.empty() || !running; });
            swap_flag = !swap_flag;
            if(swap_flag){ current=&buffer1; back=&buffer2; } else { current=&buffer2; back=&buffer1; }
            current->swap(*back);
            lock.unlock();
...
public:
    ...
    void submit(function<void()> task){
        ...
        { lock_guard<mutex> lock(queueMutex); tasks.push(move(task)); }
        condvar.notify_one();
    }
...
private:
    ...
    unordered_map<int,deque<string>> send_queues; // 待发送队列
    atomic<int> tps{0};
    size_t epoll_batch_size = 256;
...
public:
        ...
        while(running){
            ...
            for(int i=0;i<n;i++){
                ...
                if(fd == listen_fd) accept_client();
                    ...
                    if(evs & EPOLLIN) handle_client(fd);
                    if(evs & EPOLLOUT) flush_send_queue(fd);
                }
            }
        }
        close(listen_fd);
    ...
    void process_buffer(int fd){
        ...
        while((pos=buffer.find('\n'))!=string::npos){
            ...
            tps++;
            if(batch_msgs.size()>=epoll_batch_size){
    ...
    void submit_batch(int fd, const vector<string>& msgs){
        pool.submit([this, fd, msgs]{
            stringstream ss;
            ss << this_thread::get_id();
            for(auto &msg : msgs){
                g_logger.log(LogLevel::DEBUG,"[Worker] thread="+ss.str()+" fd="+to_string(fd)+" msg="+msg);
                send_queues[fd].push_back(msg + "\n");
            }
            flush_send_queue(fd);
        });
    }

    void flush_send_queue(int fd){
        auto &q = send_queues[fd];
        while(!q.empty()){
            string &msg = q.front();
...
void monitor(EpollServer &server, atomic<bool>& running, int interval){
    ...
    while(running){
        ...
        g_logger.log(LogLevel::INFO,"[Monitor] CPU="+to_string(cpuUsage)+"% Mem="+to_string(memKB/1024.0)+"MB TPS="+to_string(tps));
    }
}
...
int main(){
    ...
    ThreadPool pool(cfg.thread_num, cfg.thread_batch);
