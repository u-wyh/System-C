...
public:
    ...
    void run(){
        monitorThread=thread([&](){
            ...
            while(g_running){
                ...
                long long cur_total = total_requests.load();
                int active = active_connections.load();
                cout<<"[Monitor]: total="<<cur_total<<" TPS="<<(cur_total-last_total)<<" active="<<active<<endl;
    ...
    void handle_read(int fd){
        ...
        while(true){
            ...
            if(n>0){
                ...
                // CPU 优化：在本地复制数据再提交任务，减少锁争用
                vector<char> data(conn->readBuffer.begin(), conn->readBuffer.end());
                conn->readBuffer.clear();

                pool.submit([this, conn, data=move(data)](){
                    // 回显
                    conn->writeBuffer.insert(conn->writeBuffer.end(), data.begin(), data.end());
                    modify_write(conn->fd);
                    total_requests++;
                });
            }
    ...
    void handle_write(int fd){
        ...
        while(conn->writeOffset < conn->writeBuffer.size()){
            ssize_t n = write(fd, conn->writeBuffer.data()+conn->writeOffset,
                              conn->writeBuffer.size()-conn->writeOffset);
            if(n>0) conn->writeOffset += n;
            else{
                if(errno==EAGAIN) break;
                close_client(fd);
                return;
